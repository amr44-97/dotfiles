...
--
myLayout = avoidStruts ( (spacing 2 $ tiled) ||| Mirror tiled ||| Full)
  where 
...
myLogHook h = dynamicLogWithPP $ def
  { ppLayout = wrap "(<fc=#e4b63c>" "</fc>)"
     ...
     , ppCurrent =xmobarColor "#c792ea" "" . wrap "<box type=Bottom width=3 mb=2 color=#e309cd>" "</box>" -- Non-focused (but still visible) screen
...
--main = xmonad =<< statusBar myBar myPP toggleStrutsKey $ docks  defaults  

main :: IO ()
main =do 
    --xmproc <- spawnPipe  "xmobar"
   xmobarProc <- spawnPipe "xmobar -x 0 ~/.config/xmobar/xmobarrc"
   xmonad $ withUrgencyHook LibNotifyUrgencyHook $ docks  def{
      -- simple stuff
        terminal           = myTerminal,
