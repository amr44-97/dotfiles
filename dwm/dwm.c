...
enum { SchemeNorm, SchemeSel, SchemeHid, SchemeWarn, SchemeUrgent, SchemeStatus, SchemeTagsSel, SchemeTagsNorm, SchemeInfoSel, SchemeInfoNorm }; /* color schemes */
  ...
  int ret, i,stw = 0 ,w, x, len;
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTWM(text) - lrpad / 2 + 2 ;
			drw_text(drw, x  - stw , 0, w, bh,lrpad / 2 - 2, text, 0,True);
...
drawbar(Monitor *m)
{
	int x, w, wdelta, sw = 0, stw = 0,n = 0,scm;
		...
		if (n > 0) {
			...
			for (c = m->clients; c; c = c->next) {
				...
				if (m->sel == c)
					...
					scm = SchemeNorm;
				drw_setscheme(drw,scheme[scm]); //scheme[m == selmon ? SchemeInfoSel : scm]);
